...
<head>
 ...
 <link
   href="https://fonts.googleapis.com/css2?family=poppins:wght400;600;700&display=swap"
   rel="stylesheet"
 />
...
document.addEventListener('DOMContentLoaded', () => {
 ...
 function setupAnimation() {
   data = circles.map((c, i) => {
     const initR = initialRadii [i] * 0.95;
     return {
       el: c,
       ...
       vr: (Math.random() * 0.03 + 0.015) * (Math.random() < 0.5 ? -1 : 1), // Aumento na velocidade de variação do raio
       rmin: initR * 0.90, // Raio mínimo menor
       rmax: initR * 1.10  // Raio máximo maior
     };
   });
 }
